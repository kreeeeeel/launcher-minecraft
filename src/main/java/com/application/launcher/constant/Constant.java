package com.application.launcher.constant;

public class Constant {
    public static final int NAME_MIN = 6;
    public static final int NAME_MAX = 20;
    public static final int LOGIN_MIN = 6;
    public static final int LOGIN_MAX = 20;

    public static final int PASSWORD_MIN = 6;
    public static final int PASSWORD_MAX = 20;

    //public static final String API = "http://localhost:8745";
    public static final String API = "http://91.201.41.150:8745";
    public static final String WEB = "http://nikitapidor.com";
    public static final String RECOVERY =  "/recovery";
    public static final String PAY =  "/pay";
    public static final String ICONS = "/resources/servers-icons/";
    public static final String PHOTO = "/resources/photo/";
}
